...
def call_openai(
    ...
    # 1. Import bên trong hàm để hỗ trợ cơ chế mock test
    from openai import OpenAI

    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))

    # 2. Đo thời gian bằng perf_counter để đảm bảo độ chính xác micro giây
    start_time = time.perf_counter()

    # 3. Luôn truyền đối số dạng keyword arguments (kwargs)
    ...
    # 4. Trích xuất text và tính latency
...
def compare_models(prompt: str) -> dict:
    ...
    """
    # 1. Gọi lần lượt hai model qua các hàm đã định nghĩa
    gpt4o_response, gpt4o_latency = call_openai(prompt=prompt)
    mini_response, mini_latency = call_openai_mini(prompt=prompt)

    # 2. Ước lượng chi phí phản hồi của gpt-4o (giả định: 0.75 từ ≈ 1 token)
    word_count = len(gpt4o_response.split())
    tokens_estimate = word_count / 0.75
    gpt4o_cost_estimate = (
        (tokens_estimate / 1000) * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    )

    # 3. Trả về đúng 5 key theo yêu cầu của bài test
    return {
...
def chat_with_system_prompt(
    ...
    # Danh sách messages gồm 2 phần tử theo đúng thứ tự: system -> user
    response = client.chat.completions.create(
        model=model,
        messages=[
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Fallback ước lượng: trung bình 1 token ≈ 4 ký tự
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    """Tính toán số token và chi phí (USD) cho cả chiều input lẫn output."""
    # prompt chính là input, response chính là output
    input_tokens = count_tokens(prompt, model=model)
    output_tokens = count_tokens(response, model=model)

    rates = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Thoát không phân biệt hoa thường
        ...
        reply = ""
        for chunk in stream:
            # Chunk cuối cùng thường có delta.content là None -> dùng `or ""`
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            reply += delta
        print()  # Xuống dòng khi kết thúc câu trả lời

        # Lưu phản hồi vào history và cắt lấy 3 lượt gần nhất (6 messages)
...
def retry_with_backoff(
    ...
    """
    # max_retries + 1 lượt gồm: 1 lần gọi ban đầu + max_retries lần thử lại
    for attempt in range(max_retries + 1):
        try:
            return fn()
        except Exception:
            # Nếu đã dùng hết lượt thử lại, ném lại đúng exception gốc
...
def run_assistant(
    ...
    while True:
        # BẪY QUAN TRỌNG: Kiểm tra max_turns TRƯỚC khi gọi get_input()
        # để tránh StopIteration khi max_turns = 0 hoặc khi mock test hết input
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # Thoát phiên chat nếu gặp 'quit' hoặc 'exit'
        if user_msg.strip().lower() in ("quit", "exit"):
            break

        # Ghép messages: system (persona) luôn ở đầu + history gần nhất + prompt mới
        messages = [{"role": "system", "content": persona}] + history + [
            {"role": "user", "content": user_msg}
        ]

        # Bọc lời gọi API vào retry_with_backoff để chống rớt mạng / lỗi tạm thời
        ...
        # Stream từng chunk và ghép lại thành reply hoàn chỉnh
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
        ...
        # Cập nhật history và giữ tối đa 3 lượt (6 messages) gần nhất
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]

        # Tính toán token và chi phí cộng dồn (user + assistant)
        cost_data = estimate_cost(user_msg, reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    col_p, col_g, col_m, col_gl, col_ml = 40, 40, 40, 16, 16

    header = (
        f"{'Prompt':<{col_p}} | "
        f"{'GPT-4o Response':<{col_g}} | "
        f"{'Mini Response':<{col_m}} | "
        f"{'GPT-4o Latency':<{col_gl}} | "
        f"{'Mini Latency':<{col_ml}}"
    )
    divider = "-" * len(header)
    rows = [header, divider]

    for item in results:
        # Cắt ngắn text nếu vượt quá 40 ký tự và loại bỏ ký tự xuống dòng
        prompt_txt = item.get("prompt", "").replace("\n", " ")
        p = (prompt_txt[:37] + "...") if len(prompt_txt) > col_p else prompt_txt

        g4o_txt = item.get("gpt4o_response", "").replace("\n", " ")
        g = (g4o_txt[:37] + "...") if len(g4o_txt) > col_g else g4o_txt

        mini_txt = item.get("mini_response", "").replace("\n", " ")
        m = (mini_txt[:37] + "...") if len(mini_txt) > col_m else mini_txt

        g_lat = f"{item.get('gpt4o_latency', 0.0):.3f}s"
        m_lat = f"{item.get('mini_latency', 0.0):.3f}s"
